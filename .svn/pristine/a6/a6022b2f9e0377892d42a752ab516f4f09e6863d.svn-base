...
@Component({
    ...
    styleUrls: [
        './report-daily.component.scss',
        './report-daily.component.css'
    ],
})
export class ReportDailyComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        intDateRangePicker_daily_viewchild(Number(new Date()) );
        const self = this;
        $(function () {
            $('#startDateReportTemp').change(function () {
                self.date_ranger_event();
            });
            $('#reportrangefba00').on('apply.daterangepicker').click(function () {
                self.date_ranger_event();
            });
        });
    }
