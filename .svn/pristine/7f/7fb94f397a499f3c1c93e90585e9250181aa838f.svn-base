...
@Component({
  ...
  styleUrls: ['./zalofollower.component.scss'],
  // chỉnh css angular
...
export class ZaloFollowerComponent implements OnInit, OnDestroy {
  ...
  defaultModel = environment.API.zaloFollower;
  modalRef: BsModalRef;
  // mặc định
  canAdd: boolean;
  data: Array<any>;
  ...
  getData() {
    ...
    this.appservice.post(data, url).subscribe(res => {
      ...
      for (let i = 0; i < retrieveData.length; i++) {
        ...
        if (retrieveData[i].display_name === null) {
          retrieveData[i].display_name = '--';
        }
        if (retrieveData[i].follower_name === null) {
          retrieveData[i].follower_name = '--';
        }
      }
