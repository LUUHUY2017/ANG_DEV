...
@Component({
    templateUrl: 'effect-store.component.html',
    styleUrls: ['effect-store.component.scss'],
    // encapsulation: ViewEncapsulation.None
})
export class FootfallEffectStoreComponent implements OnInit {
    @ViewChild(StoreReportingFFScheduleComponent) scheduleComponent: StoreReportingFFScheduleComponent;
    defaultModule = 4;
    indexViewBy: number;
    currentPageId = environment.Pages.footfall.performance_store;
    ...
    page_id = environment.Pages.footfall.performance_store;
    ...
    time_value: any = '';
    menu_tree: any[];
    total_traffic2: number;
    total_num_to_enter2: number;
    all_data: any[];
    ...
    items1 = 0; items2 = 0;
    staff1 = 0; staff2 = 0;
    total_seconds1 = 0; total_seconds2 = 0;
    ...
    get_sitetree_compare() {
        this.blockUI.start(this.language.dang_tai_thong_tin_to_chuc);
        const url = environment.API.sites + '_get_site_for_report';
        const agrument = { organization_id: this.organization_id_compare };
        return new Promise(resolve => {
            this.appservice.post(agrument, url).subscribe(res => {
                if (!environment.production) {
                    console.log(res);
    ...
    function_tinh_toan(data: any, i: number) {
        ...
        // tslint:disable-next-line: max-line-length
        this['total_turn_rate' + i] = Number(this['total_passer_by' + i]) > 0 ? Number(((this['total_passer_by' + i] / this['total_passer_by' + i]) * 100).toFixed(2)) : 0;
        ...
        // tslint:disable-next-line: max-line-length
        this['tt_sales_on_sh' + i] = this['tt_sales_hour' + i] > 0 ? Number((this['tt_sales' + i] / this['tt_sales_hour' + i]).toFixed(2)) : 0;
    ...
    get_valuesuffix() {
        ...
        if (this.indexOptionSelected === this.indexess.turn_in_rate
            ...
            || this.indexOptionSelected === this.indexess.missed_member_rate || this.indexOptionSelected === this.indexess.cx_index
                    ...
                    switch (this.indexOptionSelected) {
                        ...
                        case this.indexess.avg_time:
                            ...
                            res.data.forEach(element => {
                                chart_data.push((Number(element.avg_time)));
                            });
    ...
    is_table_show_percent(indexOptionSelected) {
        if (indexOptionSelected === this.indexess.turn_in_rate || indexOptionSelected === this.indexess.conversion_rate
            || indexOptionSelected === this.indexess.member_visitors || indexOptionSelected === this.indexess.member_transactions
            || indexOptionSelected === this.indexess.member_conversion_rate || indexOptionSelected === this.indexess.missed_member_rate
            || indexOptionSelected === this.indexess.cx_index || indexOptionSelected === this.indexess.sales_hours
            || indexOptionSelected === this.indexess.sales_on_sales_hour || indexOptionSelected === this.indexess.shoppers_on_sales_hour
            || indexOptionSelected === this.indexess.nps_index || indexOptionSelected === this.indexess.sales_yield
            || indexOptionSelected === this.indexess.atv || indexOptionSelected === this.indexess.avg_items) {
            return true;
        } else {
            return false;
        }
