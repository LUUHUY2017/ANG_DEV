...
Heatmap(Highcharts);
declare var $;
declare function intDateRangePicker_monthly_viewchild(start_date): any;
@Component({
  templateUrl: 'customer-monthly.component.html',
  styleUrls: ['customer-monthly.component.scss'],
  // encapsulation: ViewEncapsulation.None
})
export class FootfallCustomerMonthlyComponent implements OnInit {
  ...
  page_id = environment.Pages.footfall.customer_monthly;
  ...
  ngOnInit() {
    this.get_page_param();
    intDateRangePicker_monthly_viewchild(Number(new Date()));
    const self = this;
    $(function () {
  ...
  date_ranger_event() {
    ...
    //     console.log(' Thời gian chọn báo cáo ' + this.start_date);
    // }
    ...
    if (selected_date != null) {
      ...
      if (_date_S.getMonth() === time_on_now.getMonth()) {
  ...
  DateParse(value: any): Date | null {
    if ((typeof value === 'string') && (value.includes('-'))) {
      ...
      const da = new Date(year, month, date);
      ...
      param => {
        ...
        try {
          ...
          if (para.start_date && para.end_date) {
            ...
            intDateRangePicker_monthly_viewchild(Number(start_d));
            this.end_date = para.end_date;
            if (start_d.getMonth() === time_on_now.getMonth()) {
              this.end_date = '\'' + this.appservice.convert_date_tostringdate_by_nghia(time_on_now) + '\'';
            }


          } else {
            intDateRangePicker_monthly_viewchild(Number(time_on_now));
            const first_d = new Date(time_on_now.getFullYear(), time_on_now.getMonth(), 1);
            const last_d = new Date(time_on_now.getFullYear(), time_on_now.getMonth() + 1, 0);
            this.start_date = '\'' + this.appservice.convert_date_tostringdate_by_nghia(first_d) + '\'';
            this.end_date = '\'' + this.appservice.convert_date_tostringdate_by_nghia(time_on_now) + '\'';
          }
  ...
  get_data() {
    ...
    this.appservice.post(data, environment.API.sp_report_poc_raw_data_by_month).subscribe(
      res => {
        try {
  ...
  exportExcel(): void {
    ...
    this.appservice.post(data, environment.API.sp_report_poc_raw_data_by_month + '_export_excel').subscribe(fileData => {
    ...
    try {
      ...
      if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        if (window.navigator.msSaveBlob) {
          ...
          navigator.msSaveBlob(blob, 'Monthly Traffic Report on ' + _date_SS + '.xls');
        }
      } else {
        const downloadLink = document.createElement('a');
        document.body.appendChild(downloadLink);
        downloadLink.href = data_type + ', ' + encodeURIComponent(tab_text);
        downloadLink.download = 'Monthly Traffic Report on ' + _date_SS + '.xls';
        downloadLink.click();
      }
    } catch (error) {
