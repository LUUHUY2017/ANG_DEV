...
export class FootfallMetricsTrendComponent implements OnInit {
  ...
  defaultModule = 1;
  indexViewBy: number;
  currentPageId = environment.Pages.footfall.metric_trend;
  ...
  end_date: any;
  time_value = '';
  public page_id = environment.Pages.footfall.metric_trend;
  ...
  menu_tree: any[];
  order: number;
  total_passer_by = 0; total_seconds = 0;
  total_shopper_visits = 0;
  total_kids_visits: number;
  total_turn_in_rate: number;
  t_f: any;
  indexes: any; index_value: any; language_index_vn: any;
  ...
  set_default(defaultOrgId: any) {
    ...
    // const value_index = this.dropdownList.find(item => item.value === indexes.shoppers).value;
    ...
    Highcharts.chart(this.container.nativeElement, {
      ...
      series: [
        ...
        },
        {
          ...
          // tslint:disable-next-line: max-line-length
          yAxis: selft.set_position(this.index_value.exits), data: num_to_exit, showInLegend: selft.check_has_index(this.index_value.exits),
          visible: selft.check_has_index(this.index_value.exits),
        }, {
